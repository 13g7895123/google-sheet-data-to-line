...
VPS (port 80/443)
     │
  [Nginx] ← frontend 容器（nginx:alpine）
     │
     ├── /          靜態檔案（React build）
     └── /api/*  ─▶ [Backend] 容器（oven/bun）port 3000
                          │
                    [Volume: db_data]
...
- 已完成 [LINE Messaging API](#1-line-messaging-api) 與 [Google Sheets API](#2-google-sheets-apiservice-account) 設定（見下方章節）
...
curl http://localhost/health  # 應回傳 {"ok":true}
```

#### Step 5：設定 Nginx + HTTPS（Certbot）

Docker 的 frontend 容器只監聽 port 80，需在 **VPS 外層**再加一層 Nginx 處理 HTTPS：
...
> 若不想雙層 Nginx，可將 `docker-compose.yml` 的 frontend port 改為 `"443:443"` 並在容器內掛載憑證，但用 Certbot + 外層 Nginx 更易維護。
